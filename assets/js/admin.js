...
jQuery(function () {
	avatorElementAdmin.assetsManager.fontManager.init();
	avatorElementAdmin.roleManager.advancedRoleManager.init();
});
...
module.exports = function () {
	...
	self.init = function () {
		jQuery(document).on('change', '.elementor-widget-template-select', function () {
			...
			var editUrl = AvatorElementConfig.i18n.home_url + '?p=' + templateID + '&elementor';
...
module.exports = function (key, fieldID) {
	...
	self.cacheElements = function () {
		this.cache = {
			$button: jQuery('#avator_element_' + key + '_button'),
			$apiKeyField: jQuery('#avator_element_' + key),
			$apiUrlField: jQuery('#avator_element_' + fieldID)
		};
	};
...
module.exports = function () {
	...
	self.cacheElements = function () {
		this.cache = {
			$button: jQuery('#avator_element_typekit_validate_button'),
			$kitIdField: jQuery('#elementor_typekit-kit-id'),
			$dataLabelSpan: jQuery('.avator-element-typekit-data')
		};
	};
	self.bindEvents = function () {
		this.cache.$button.on('click', function (event) {
	...
	self.fetchFonts = function () {
		...
		jQuery.post(ajaxurl, {
			action: 'avator_element_admin_fetch_fonts',
			kit_id: kitID,
			_nonce: self.cache.$button.data('nonce')
		}).done(function (data) {
			if (data.success) {
...
var _class = function (_elementorModules$Vie) {
	...
	_createClass(_class, [{
		...
		value: function getDefaultSettings() {
			return {
				selectors: {
					button: '#avator_element_fa_pro_validate_button',
