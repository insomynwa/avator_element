...
var AvatorElementFrontend = function (_elementorModules$Vie) {
	_inherits(AvatorElementFrontend, _elementorModules$Vie);

	function AvatorElementFrontend() {
		_classCallCheck(this, AvatorElementFrontend);

		return _possibleConstructorReturn(this, (AvatorElementFrontend.__proto__ || Object.getPrototypeOf(AvatorElementFrontend)).apply(this, arguments));
	}

	_createClass(AvatorElementFrontend, [{
		key: 'onInit',
		value: function onInit() {
			_get(AvatorElementFrontend.prototype.__proto__ || Object.getPrototypeOf(AvatorElementFrontend.prototype), 'onInit', this).call(this);

			this.config = AvatorElementFrontendConfig;
	...
	return AvatorElementFrontend;
}(elementorModules.ViewModule);

window.avatorElementFrontend = new AvatorElementFrontend();
...
var _class = function (_elementorModules$Mod) {
	...
	function _class() {
		...
		avatorElementFrontend.modules.linkActions.addAction('popup:open', _this.showPopup.bind(_this));

		avatorElementFrontend.modules.linkActions.addAction('popup:close', _this.closePopup.bind(_this));
		...
		value: function showModal(avoidMultiple) {
			...
			if (!this.isEdit) {
				if (!elementorFrontend.isWPPreviewMode()) {
					...
					if (avoidMultiple && avatorElementFrontend.modules.popup.popupPopped && settings.avoid_multiple_popups) {
						return;
					}
				}
			...
			avatorElementFrontend.modules.popup.popupPopped = true;
		}
	}, {
...
var PopupFormActions = elementorModules.frontend.handlers.Base.extend({
	...
	handleFormAction: function handleFormAction(event, response) {
		...
		if ('open' === popupSettings.action) {
			return avatorElementFrontend.modules.popup.showPopup(popupSettings);
		}
		setTimeout(function () {
			return avatorElementFrontend.modules.popup.closePopup(popupSettings, event);
		}, 1000);
	}
});
...
module.exports = elementorModules.frontend.handlers.Base.extend({
	...
	getDefaultSettings: function getDefaultSettings() {
		return {
			...
			action: 'avator_element_forms_send_form',
			ajaxUrl: avatorElementFrontend.config.ajaxurl
		};
	},
...
var config = AvatorElementFrontendConfig.facebook_sdk,
	...
	bindEvents: function bindEvents() {
		...
		$toggle.on('click', function (event) {
			var currentUrl = window.location.origin + window.location.pathname,
			    isCart = AvatorElementFrontendConfig.menu_cart.cart_page_url === currentUrl,
			    isCheckout = AvatorElementFrontendConfig.menu_cart.checkout_page_url === currentUrl;

			if (!isCart && !isCheckout) {
				event.preventDefault();
				$container.toggleClass(classes.isShown);
			} else {
				var cartUrl = AvatorElementFrontendConfig.menu_cart.cart_page_url;
