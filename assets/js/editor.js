...
module.exports = ElementEditorModule.extend({
	...
	fetchCache: function fetchCache(type, cacheKey, requestArgs) {
		...
		return avatorElement.ajax.addRequest('forms_panel_action_data', {
		...
		value: function onRender() {
			...
			this.children.each(function (child) {
				...
				var $imageWrapper = jQuery('<div>', { class: 'elementor-popup__display-settings_controls_group__icon' }),
				    $image = jQuery('<img>', { src: avatorElement.config.urls.modules + ('popup/assets/images/' + name + '/' + groupName + '.svg') });
...
module.exports = elementor.modules.controls.Repeater.extend({
	...
	initialize: function initialize() {
		...
		this.config = avatorElement.config.theme_builder;
	...
	checkConflicts: function checkConflicts(model) {
		...
		avatorElement.ajax.addRequest('theme_builder_conditions_check_conflicts', {
			unique_id: rowId,
			data: {
	...
	onRender: function onRender() {
		this.ui.btnAddRow.text(avatorElement.translate('add_condition'));
...
var AvatorElement = Marionette.Application.extend({
	config: {},
	...
	onElementorInit: function onElementorInit() {
		avatorElement.libraryRemoveGetProButtons();

		elementor.debug.addURLToWatch('avator-element/assets');
	},
	...
	libraryRemoveGetProButtons: function libraryRemoveGetProButtons() {
		elementor.hooks.addFilter('elementor/editor/template-library/template/action-button', function (viewID, templateData) {
			return templateData.isPro && !avatorElement.config.isActive ? '#tmpl-avator-element-template-library-activate-license-button' : '#tmpl-elementor-template-library-insert-button';
		});
	}
});

window.avatorElement = new AvatorElement();
	...
	_createClass(_class, [{
		...
		value: function onNavigatorInit() {
			elementor.navigator.indicators.customCSS = {
				...
				title: avatorElement.translate('custom_css'),
	...
	_createClass(_class, [{
		...
		value: function onNavigatorInit() {
			elementor.navigator.indicators.motionFX = {
				icon: 'flash',
				title: avatorElement.translate('motion_effects'),
		...
		_createClass(Module, [{
			...
			value: function addPublishTabs() {
				...
				jQuery.each(this.displaySettingsTypes, function (type, data) {
					...
					component.addTab(type, {
						...
						name: type,
						title: avatorElement.translate(type),
						description: avatorElement.translate('popup_publish_screen_' + type + '_description'),
						image: avatorElement.config.urls.modules + ('popup/assets/images/' + type + '-tab.svg')
					});
				});
			}
		}, {
			...
			value: function addLibraryTab() {
				...
				library.addTab('templates/popups', {
					title: avatorElement.translate('popups'),
					filter: {
						source: 'remote',
						type: 'popup'
					}
					...
					if (!introduction) {
						introduction = new elementorModules.editor.utils.Introduction({
							...
							dialogOptions: {
								...
								headerMessage: '<i class="eicon-info"></i>' + avatorElement.translate('popup_settings_introduction_title'),
								message: avatorElement.translate('popup_settings_introduction_message'),
								closeButton: true,
			...
			value: function onEditorSave() {
				...
				avatorElement.ajax.addRequest('popup_save_display_settings', {
					data: {
						settings: settings
					}
				});
...
var _class = function (_elementorModules$edi) {
	...
	_createClass(_class, [{
		...
		value: function onElementorPanelInit() {
			...
			avatorElement.modules.themeBuilder.on('publish-layout:init', this.addDisplaySettingsPublishScreens.bind(this));
		...
		value: function onPanelFooterSubmenuItemClick(type) {
			avatorElement.modules.themeBuilder.showPublishModal();

			avatorElement.modules.themeBuilder.getPublishLayout().modalContent.currentView.showScreenByName(type);
		}
	}]);
...
module.exports = elementor.modules.controls.Select2.extend({
	...
	getSelect2Placeholder: function getSelect2Placeholder() {
		return {
			id: '',
			text: avatorElement.translate('all')
		};
	},
		...
		return jQuery.extend(elementor.modules.controls.Select2.prototype.getSelect2DefaultOptions.apply(this, arguments), {
			ajax: {
				transport: function transport(params, success, failure) {
					...
					return avatorElement.ajax.addRequest(action, {
						data: data,
						success: success,
						error: failure
					});
	...
	var updateReplyToOptions = function updateReplyToOptions() {
		...
		emailFields = _.map(emailModels, function (model) {
			return {
				...
				label: avatorElement.translate('x_field', [model.get('field_label')])
			};
		});
...
module.exports = elementorModules.editor.utils.Module.extend({
	...
	getDataSettings: function getDataSettings(item) {
		var config = avatorElement.config.forms[item.field_type],
	...
	onInit: function onInit() {
		elementor.hooks.addFilter('avator_element/forms/content_template/item', this.filterItem);
		elementor.hooks.addFilter('avator_element/forms/content_template/field/recaptcha', this.renderField, 10, 2);
		elementor.hooks.addFilter('avator_element/forms/content_template/field/recaptcha_v3', this.renderField, 10, 2);
	}
});
...
module.exports = elementorModules.editor.utils.Module.extend({
	...
	onInit: function onInit() {
		elementor.hooks.addFilter('avator_element/forms/content_template/field/time', this.renderField, 10, 4);
	}
});
...
module.exports = elementorModules.editor.utils.Module.extend({
	...
	onInit: function onInit() {
		elementor.hooks.addFilter('avator_element/forms/content_template/field/date', this.renderField, 10, 4);
	}
});
...
module.exports = elementorModules.editor.utils.Module.extend({
	...
	onInit: function onInit() {
		elementor.hooks.addFilter('avator_element/forms/content_template/field/acceptance', this.renderField, 10, 4);
	}
});
...
module.exports = elementorModules.editor.utils.Module.extend({
	...
	onInit: function onInit() {
		elementor.hooks.addFilter('avator_element/forms/content_template/field/upload', this.renderField, 10, 4);
	}
});
...
module.exports = elementorModules.editor.utils.Module.extend({
	...
	onInit: function onInit() {
		elementor.hooks.addFilter('avator_element/forms/content_template/field/tel', this.renderField, 10, 4);
	}
});
	...
	self.onTemplateIdChange = function () {
		...
		if ($editButton.length) {
			...
			$editButton = jQuery('<a />', {
				...
				href: editUrl,
				html: '<i class="eicon-pencil" /> ' + avatorElement.config.i18n.edit_template
			});
	...
	module.exports = elementorModules.editor.utils.Module.extend({
		...
		registerTemplateType: function registerTemplateType() {
			elementor.templates.registerTemplateType('widget', {
				showInLibrary: false,
				saveDialog: {
					title: avatorElement.translate('global_widget_save_title'),
					description: avatorElement.translate('global_widget_save_description')
				},
		...
		addSavedWidgetsToPanel: function addSavedWidgetsToPanel() {
			...
			_.each(avatorElement.config.widget_templates, function (templateArgs, id) {
		...
		onWidgetTemplateSaved: function onWidgetTemplateSaved(data) {
			elementor.history.history.startItem({
				...
				type: avatorElement.translate('linked_to_global')
			});
...
GlobalWidgetView = WidgetView.extend({
	...
	initialize: function initialize() {
		...
		if (previewSettings) {
			...
			if (!globalSettingsLoadedStatus) {
				...
				avatorElement.modules.globalWidget.requestGlobalModelSettings(globalModel);
			}
	...
	getGlobalModel: function getGlobalModel() {
		if (!this.globalModel) {
			this.globalModel = avatorElement.modules.globalWidget.getGlobalModels(this.model.get('templateID'));
		}
	...
	unlink: function unlink() {
		...
		elementor.history.history.startItem({
			...
			type: avatorElement.translate('unlink_widget')
		});
...
module.exports = elementor.modules.elements.models.Element.extend({
	...
	getGlobalModel: function getGlobalModel() {
		...
		return avatorElement.modules.globalWidget.getGlobalModels(templateID);
	},
	...
	getTitle: function getTitle() {
		...
		var global = avatorElement.translate('global');
...
module.exports = Marionette.ItemView.extend({
	...
	buildUnlinkDialog: function buildUnlinkDialog() {
		...
		return elementorCommon.dialogsManager.createWidget('confirm', {
			...
			headerMessage: avatorElement.translate('unlink_widget'),
			message: avatorElement.translate('dialog_confirm_unlink'),
			position: {
				my: 'center center',
				at: 'center center'
			},
			strings: {
				confirm: avatorElement.translate('unlink'),
				cancel: avatorElement.translate('cancel')
			},
	...
	onEditButtonClick: function onEditButtonClick() {
		...
		avatorElement.modules.globalWidget.requestGlobalModelSettings(editedModel, function () {
...
module.exports = elementorModules.editor.utils.Module.extend({
	config: avatorElement.config.shareButtonsNetworks,
...
module.exports = elementorModules.Module.extend({
	...
	getCustomFont: function getCustomFont(fontType, font) {
		avatorElement.ajax.addRequest('assets_manager_panel_action_data', {
...
if (!elementorAvatorEditorConfig.useComponentsRouter) {
	...
	module.exports = elementorModules.editor.utils.Module.extend({
		...
		onElementorInit: function onElementorInit() {
			if (!avatorElement.config.theme_builder) {
				return;
			}
		...
		setConditionsModel: function setConditionsModel() {
			var themeBuilderModuleConfig = avatorElement.config.theme_builder,
		...
		onEditorSave: function onEditorSave() {
			...
			avatorElement.ajax.addRequest('theme_builder_save_conditions', {
				data: this.conditionsModel.toJSON({ remove: ['default'] }),
				success: function success() {
					avatorElement.config.theme_builder.settings.conditions = _this.conditionsModel.get('conditions');
				}
			});
		}
	});
...
module.exports = elementorModules.editor.utils.Module.extend({
	...
	addConditionsScreen: function addConditionsScreen(publishLayout) {
		...
		publishLayout.addScreen({
			...
			name: 'conditions',
			title: avatorElement.translate('conditions'),
			description: avatorElement.translate('conditions_publish_screen_description'),
			image: avatorElement.config.urls.modules + 'theme-builder/assets/images/conditions-tab.svg'
		});
	},
	...
	onEditorSave: function onEditorSave() {
		...
		avatorElement.ajax.addRequest('theme_builder_save_conditions', {
			data: this.conditionsModel.toJSON({ remove: ['default'] }),
			success: function success() {
				avatorElement.config.theme_builder.settings.conditions = _this2.conditionsModel.get('conditions');
			}
		});
	}
});
...
var _class = function (_elementorModules$com) {
	...
	_createClass(_class, [{
		...
		value: function getLogoOptions() {
			return {
				title: avatorElement.translate('publish_settings')
			};
		}
	}, {
		key: 'initModal',
		value: function initModal() {
			...
			this.modal.addButton({
				name: 'publish',
				text: avatorElement.translate('save_and_close'),
				callback: function callback() {
			...
			this.modal.addButton({
				name: 'next',
				text: avatorElement.translate('next'),
				callback: function callback() {
...
module.exports = SaverBehavior.extend({
	...
	toggleMenuConditions: function toggleMenuConditions() {
		this.ui.menuConditions.toggle(!!avatorElement.config.theme_builder.settings.location);
	},
	...
	onRender: function onRender() {
		...
		this.ui.menuConditions = this.view.addSubMenuItem('saver-options', {
			...
			name: 'conditions',
			icon: 'eicon-flow',
			title: avatorElement.translate('display_conditions'),
			callback: function callback() {
				return avatorElement.modules.themeBuilder.showPublishModal();
			}
		});
	...
	onChangeLocation: function onChangeLocation(settings) {
		if (!_.isUndefined(settings.changed.location)) {
			avatorElement.config.theme_builder.settings.location = settings.changed.location;
			this.toggleMenuConditions();
		}
	},

	onClickButtonPublish: function onClickButtonPublish() {
		var hasConditions = avatorElement.config.theme_builder.settings.conditions.length,
		    hasLocation = avatorElement.config.theme_builder.settings.location,
