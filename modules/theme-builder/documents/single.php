<?php
namespace AvatorElement\Modules\ThemeBuilder\Documents;
...
class Single extends Theme_Page_Document {
	...
	public static function get_title() {
		return __( 'Single', 'avator-element' );
	}
	...
	protected static function get_editor_panel_categories() {
		$categories = [
			'theme-elements-single' => [
				'title' => __( 'Single', 'avator-element' ),
			],
		];
	...
	public function print_content() {
		...
		if ( $requested_post_id !== $this->post->ID ) {
			...
			if ( $requested_document && ! $requested_document instanceof Section && $requested_document->get_location() !== $this->get_location() ) {
				echo '<div class="elementor-theme-builder-content-area">' . __( 'Content Area', 'avator-element' ) . '</div>';
	...
	public static function get_preview_as_options() {
		...
		return [
			'single' => [
				'label' => __( 'Single', 'avator-element' ),
				'options' => $post_types_options,
			],
			'page/404' => __( '404', 'avator-element' ),
		];
	}
	...
	public function preview_error_handler() {
		...
		wp_localize_script( 'elementor-frontend', 'elementorPreviewErrorArgs', [
			...
			'headerMessage' => sprintf( __( 'The %s Widget was not found in your template.', 'avator-element' ), $depended_widget_title ),
			/* translators: %1$s: is the widget name. %2$s: is the template name.  */
			'message' => sprintf( __( 'You must include the %1$s Widget in your template (%2$s), in order for Elementor to work on this page.', 'avator-element' ), $depended_widget_title, '<strong>' . static::get_title() . '</strong>' ),
			'strings' => [
				'confirm' => __( 'Edit Template', 'avator-element' ),
			],
