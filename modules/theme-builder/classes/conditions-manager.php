<?php
namespace AvatorElement\Modules\ThemeBuilder\Classes;
...
use AvatorElement\Modules\ThemeBuilder\Conditions\Condition_Base;
...
class Conditions_Manager {
	...
	public function admin_columns_headers( $posts_columns ) {
		...
		$posts_columns = array_slice( $posts_columns, 0, $offset, true ) + [
			'instances' => __( 'Instances', 'avator-element' ),
	...
	public function ajax_check_conditions_conflicts( $request ) {
		...
		/** @var \AvatorElement\Modules\ThemeBuilder\Module $theme_builder_module */
		$theme_builder_module = Module::instance();
		...
		if ( ! empty( $conflicted ) ) {
			$message = __( 'Elementor recognized that you have set this location for other templates: ', 'avator-element' ) . ' ' . implode( ', ', $conflicted );
		}
	...
	private function register_condition( $id, $args = [] ) {
		...
		$class_name = '\\AvatorElement\\Modules\\ThemeBuilder\\Conditions\\' . $class_name;
