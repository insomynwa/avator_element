...
class Number extends Field_Base {
	...
	public function get_name() {
		return __( 'Number', 'avator-element' );
	}
	...
	public function update_controls( $widget ) {
		...
		$field_controls = [
			'field_min' => [
				'name' => 'field_min',
				'label' => __( 'Min. Value', 'avator-element' ),
			...
			'field_max' => [
				'name' => 'field_max',
				'label' => __( 'Max. Value', 'avator-element' ),
				'type' => Controls_Manager::NUMBER,
				'condition' => [
					'field_type' => $this->get_type(),
				],
	...
	public function validation( $field, Classes\Form_Record $record, Classes\Ajax_Handler $ajax_handler ) {
		...
		if ( ! empty( $field['field_max'] ) && $field['field_max'] < (int) $field['value'] ) {
			$ajax_handler->add_error( $field['id'], sprintf( __( 'The value must be less than or equal to %s', 'avator-element' ), $field['field_max'] ) );
		}

		if ( ! empty( $field['field_min'] ) && $field['field_min'] > (int) $field['value'] ) {
			$ajax_handler->add_error( $field['id'], sprintf( __( 'The value must be greater than or equal %s', 'avator-element' ), $field['field_min'] ) );
		}
	}
