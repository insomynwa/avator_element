...
class Recaptcha_Handler {
	...
	public static function get_setup_message() {
		return __( 'To use reCAPTCHA, you need to add the API Key and complete the setup process in Dashboard > Elementor > Settings > Integrations > reCAPTCHA.', 'avator-element' );
	}

	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, static::get_recaptcha_name(), [
			'label' => __( 'reCAPTCHA', 'avator-element' ),
			'callback' => function () {
				echo sprintf( __( '<a href="%s" target="_blank">reCAPTCHA</a> is a free service by Google that protects your website from spam and abuse. It does this while letting your valid users pass through with ease.', 'avator-element' ), 'https://www.google.com/recaptcha/' );
			},
			'fields' => [
				'pro_recaptcha_site_key' => [
					'label' => __( 'Site Key', 'avator-element' ),
					'field_args' => [
						'type' => 'text',
					],
				],
				'pro_recaptcha_secret_key' => [
					'label' => __( 'Secret Key', 'avator-element' ),
					'field_args' => [
						'type' => 'text',
					],
				],
	...
	public function register_scripts() {
		...
		wp_register_script( $script_name, $src, [], AVATOR_ELEMENT_VERSION, true );
	}
	...
	public function validation( $record, $ajax_handler ) {
		...
		if ( empty( $_POST['g-recaptcha-response'] ) ) {
			$ajax_handler->add_error( $field['id'], __( 'The Captcha field cannot be blank. Please enter a value.', 'avator-element' ) );

			return;
		}

		$recaptcha_errors = [
			'missing-input-secret' => __( 'The secret parameter is missing.', 'avator-element' ),
			'invalid-input-secret' => __( 'The secret parameter is invalid or malformed.', 'avator-element' ),
			'missing-input-response' => __( 'The response parameter is missing.', 'avator-element' ),
			'invalid-input-response' => __( 'The response parameter is invalid or malformed.', 'avator-element' ),
		];
		...
		if ( 200 !== (int) $response_code ) {
			...
			$ajax_handler->add_error( $field['id'], sprintf( __( 'Can not connect to the reCAPTCHA server (%d).', 'avator-element' ), $response_code ) );
		...
		if ( ! $this->validate_result( $result, $field ) ) {
			$message = __( 'Invalid Form - reCAPTCHA validation failed', 'avator-element' );
	...
	public function add_field_type( $field_types ) {
		$field_types['recaptcha'] = __( 'reCAPTCHA', 'avator-element' );
	...
	public function __construct() {
		...
		add_action( 'avator_element/forms/render_field/' . static::get_recaptcha_name(), [ $this, 'render_field' ], 10, 3 );
		add_filter( 'avator_element/forms/render/item', [ $this, 'filter_field_item' ] );
