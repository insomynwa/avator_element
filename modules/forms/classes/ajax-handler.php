...
class Ajax_Handler {
	...
	public static function is_form_submitted() {
		return wp_doing_ajax() && isset( $_POST['action'] ) && 'avator_element_forms_send_form' === $_POST['action'];
	}

	public static function get_default_messages() {
		return [
			self::SUCCESS => __( 'The form was sent successfully.', 'avator-element' ),
			self::ERROR => __( 'An error occured.', 'avator-element' ),
			self::FIELD_REQUIRED => __( 'This field is required.', 'avator-element' ),
			self::INVALID_FORM => __( 'There\'s something wrong. The form is invalid.', 'avator-element' ),
			self::SERVER_ERROR => __( 'Server error. Form not sent.', 'avator-element' ),
			self::SUBSCRIBER_ALREADY_EXISTS => __( 'Subscriber already exists.', 'avator-element' ),
		];
	}
	...
	public static function get_default_message( $id, $settings ) {
		...
		return isset( $default_messages[ $id ] ) ? $default_messages[ $id ] : __( 'Unknown', 'avator-element' );
	}
	...
	public function ajax_send_form() {
		 ...
		 */
		do_action( 'avator_element/forms/new_record', $record, $this );
	...
	public function send() {
		...
		if ( current_user_can( 'edit_post', $_POST['post_id'] ) && ! empty( $this->messages['admin_error'] ) ) {
			$this->add_admin_error_message( __( 'This Message is not visible for site visitors.', 'avator-element' ) );
	...
	public function __construct() {
		add_action( 'wp_ajax_avator_element_forms_send_form', [ $this, 'ajax_send_form' ] );
		add_action( 'wp_ajax_nopriv_avator_element_forms_send_form', [ $this, 'ajax_send_form' ] );
	}
}
