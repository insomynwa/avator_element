...
class Slack extends Action_Base {
	...
	public function get_label() {
		return __( 'Slack', 'avator-element' );
	}
	...
	public function register_settings_section( $widget ) {
		...
		$widget->add_control(
			...
			[
				...
				'label_block' => true,
				'separator' => 'before',
				'description' => __( 'Enter the webhook URL that will receive the form\'s submitted data.', 'avator-element' ) . ' ' . sprintf( '<a href="%s" target="_blank">%s</a>.', 'https://slack.com/apps/A0F7XDUAZ-incoming-webhooks/', __( 'Click here for Instructions', 'avator-element' ) ),
				'render_type' => 'none',
		...
		$widget->add_control(
			'slack_channel',
			[
				'label' => __( 'Channel', 'avator-element' ),
				'type' => Controls_Manager::TEXT,
			]
		);
		...
		$widget->add_control(
			'slack_pretext',
			[
				'label' => __( 'Pre Text', 'avator-element' ),
				'type' => Controls_Manager::TEXT,
			]
		);
	...
	public function run( $record, $ajax_handler ) {
		...
		$attachment = [
			'text' => __( 'A new Form Submission has been received', 'avator-element' ),
			'title' => __( 'A new Submission', 'avator-element' ),
		...
		if ( ! empty( $settings['slack_add_ts'] ) && 'yes' === $settings['slack_add_ts'] ) {
			$attachment = array_merge( $attachment, [
				'footer' => sprintf( __( 'Powered by %s', 'avator-element' ), 'Elementor' ),
		...
		$webhook_data = apply_filters( 'avator_element/forms/slack/webhook_args', $webhook_data );
