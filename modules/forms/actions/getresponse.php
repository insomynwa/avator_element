...
use AvatorElement\Modules\Forms\Classes\Getresponse_Handler;
...
class Getresponse extends Integration_Base {
	...
	public function get_label() {
		return __( 'GetResponse', 'avator-element' );
	}
	...
	public function register_settings_section( $widget ) {
		$widget->start_controls_section(
			'section_getresponse',
			[
				'label' => __( 'GetResponse', 'avator-element' ),
				'condition' => [
		...
		$widget->add_control(
			...
			[
				'label' => __( 'Day Of Cycle', 'avator-element' ),
	...
	public function run( $record, $ajax_handler ) {
		...
		if ( ! $subscriber ) {
			$ajax_handler->add_admin_error_message( __( 'GetResponse Integration requires an email field', 'avator-element' ) );
	...
	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'getresponse', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'GetResponse', 'avator-element' ) . '</h2>';
			},
			'fields' => [
				self::OPTION_NAME_API_KEY => [
					...
					'field_args' => [
						'type' => 'text',
						'desc' => sprintf( __( 'To integrate with our forms you need an <a href="%s" target="_blank">API Key</a>.', 'avator-element' ), 'https://www.getresponse.com' ),
					],
				],
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-action="%s" data-nonce="%s" class="button elementor-button-spinner" id="avator_element_getresponse_api_key_button">%s</button>', self::OPTION_NAME_API_KEY . '_validate', wp_create_nonce( self::OPTION_NAME_API_KEY ), __( 'Validate API Key', 'avator-element' ) ),
					],
				],
			],
		] );
