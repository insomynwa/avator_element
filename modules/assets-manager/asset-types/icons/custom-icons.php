...
class Custom_Icons extends  Assets_Base {
	...
	public function get_name() {
		return __( 'Custom Icons', 'avator-element' );
	}
	...
	public function render_metabox( $post ) {
		...
		$fields = [
			...
			[
				...
				'label' => false,
				'sub-label' => __( 'Your Fontello, IcoMoon or Fontastic .zip file', 'avator-element' ),
			],
			[
				'id' => 'close_div',
				'field_type' => 'html_tag',
	...
	public static function get_supported_icon_sets() {
		...
		return array_merge( apply_filters( 'avator_element/icons_manager/custom_icons/additional_supported_types', [] ), $icon_sets );
	}
	...
	private function upload() {
		...
		if ( 'zip' !== $ext ) {
			...
			return new \WP_Error( 'unsupported_file', __( 'Only zip files are allowed', 'avator-element' ) );
		}
	...
	private function upload_and_extract_zip() {
		...
		if ( count( $source_files ) === 0 ) {
			return new \WP_Error( 'incompatible_archive', esc_html__( 'Incompatible archive', 'avator-element' ) );
		}
	...
	public function custom_icons_upload_handler( $data ) {
		...
		return new \WP_Error( 'unsupported_zip_format', __( 'The zip file provided is not supported!', 'avator-element' ) );
	}
	...
	public function manage_columns( $columns ) {
		return [
			...
			'title' => __( 'Icon Set', 'avator-element' ),
			'icons_prefix' => __( 'CSS Prefix', 'avator-element' ),
		];
	}

	public function update_enter_title_here( $title, $post ) {
		if ( isset( $post->post_type ) && Icons_Manager::CPT === $post->post_type ) {
			return __( 'Enter Icon Set Name', 'avator-element' );
		}
