<?php
namespace AvatorElement\Modules\AssetsManager\AssetTypes\Fonts;

use Elementor\Core\Files\CSS\Base;
use AvatorElement\Modules\AssetsManager\Classes;
use AvatorElement\Modules\AssetsManager\AssetTypes\Fonts_Manager;
...
class Custom_Fonts extends Classes\Font_Base {
	...
	public function get_name() {
		return __( 'Custom Fonts', 'avator-element' );
	}
	...
	public function add_meta_box() {
		add_meta_box(
			...
			__( 'Manage Your Font Files', 'avator-element' ),
			[ $this, 'render_metabox' ],
			Fonts_Manager::CPT,
			'normal',
			'default'
	...
	public function render_metabox( $post ) {
		...
		$fields = [
			...
			[
				...
				'field_type' => 'html',
				'label' => false,
				'raw_html' => sprintf( '<div class="inline-preview">%s</div>', __( 'Elementor Is Making the Web Beautiful!!!', 'avator-element' ) ),
			],
			[
				'id' => 'toolbar',
				'field_type' => 'toolbar',
		...
		foreach ( $this->get_file_types() as $type => $mine ) {
			$fields[] = [
				...
				'label' => sprintf( __( '%s File', 'avator-element' ), strtoupper( $type ) ),
				/* translators: %s: Font file format. */
				'box_title' => sprintf( __( 'Upload font .%s file', 'avator-element' ), $type ),
				/* translators: %s: Font file format. */
				'box_action' => sprintf( __( 'Select .%s file', 'avator-element' ), $type ),
				'preview_anchor' => 'none',
		...
		$repeater = [
			...
			'id' => 'font_face',
			'label' => false,
			'add_label' => __( 'Add Font Variation', 'avator-element' ),
			'toggle_title' => __( 'Edit', 'avator-element' ),
			'remove_title' => __( 'Delete', 'avator-element' ),
			'field_type' => 'repeater',
			'row_label' => [
				'default' => 'Settings',
	...
	public function wp_handle_upload_prefilter( $file ) {
		...
		if ( $svg_handler::svg_sanitizer_can_run() && ! $svg_handler->sanitize_svg( $file['tmp_name'] ) ) {
			$file['error'] = __( 'Invalid SVG Format, file not uploaded for security reasons', 'avator-element' );
		}
	...
	public function get_font_face_from_data( $font_family, $data ) {
		...
		$font_face .= "\tfont-display: " . apply_filters( 'avator_element/custom_fonts/font_display', 'auto', $font_family, $data ) . ';' . PHP_EOL;
	...
	public function handle_panel_request( array $data ) {
		...
		if ( empty( $font_face ) ) {
			...
			$error_message = sprintf( __( 'Font %s was not found.', 'avator-element' ), $font_family );
	...
	private function get_font_style_options() {
		return [
			...
			'italic' => __( 'Italic', 'avator-element' ),
			'oblique' => __( 'Oblique', 'avator-element' ),
		];
	}
	...
	private function get_file_type_description( $file_type ) {
		$descriptions = [
			'eot' => __( 'Embedded OpenType, Used by IE6-IE9 Browsers', 'avator-element' ),
			'woff2' => __( 'The Web Open Font Format 2, Used by Super Modern Browsers', 'avator-element' ),
			'woff' => __( 'The Web Open Font Format, Used by Modern Browsers', 'avator-element' ),
			'ttf' => __( 'TrueType Fonts, Used for better supporting Safari, Android, iOS', 'avator-element' ),
			'svg' => __( 'SVG fonts allow SVG to be used as glyphs when displaying text, Used by Legacy iOS', 'avator-element' ),
		];
