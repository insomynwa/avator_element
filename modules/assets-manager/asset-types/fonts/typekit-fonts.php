...
use AvatorElement\Modules\AssetsManager\Classes\Font_Base;
...
class Typekit_Fonts extends Font_Base {
	...
	public function get_name() {
		return __( 'TypeKit Web Fonts by Adobe', 'avator-element' );
	}
	...
	private function fetch_typekit_data() {
		...
		if ( 200 !== $response_code ) {
			switch ( $response_code ) {
				case 404:
					$this->error = __( 'Project not found.', 'avator-element' );
					break;
				default:
		...
		if ( ! $response_body ) {
			$this->error = __( 'No project data was returned.', 'avator-element' );
	...
	public function handle_panel_request( array $data ) {
		...
		if ( ! $typekit_fonts || ! is_array( $typekit_fonts ) ) {
			throw new \Exception( __( 'Error with TypeKit fonts', 'avator-element' ) );
		}

		if ( ! in_array( $font_family, array_keys( $typekit_fonts ) ) ) {
			throw new \Exception( __( 'Font missing in Project', 'avator-element' ) );
		}
	...
	public function register_admin_fields( Settings $settings ) {
		...
		$button_label = __( 'Get Project ID', 'avator-element' );
		$found_label = '<span class="avator-element-typekit-count">{{count}}</span> ' . __( 'Fonts Families Found in project. Please note that typekit takes a few minutes to sync once you publish or update a project.', 'avator-element' );
		if ( $fonts && is_array( $fonts ) ) {
			$button_label = __( 'Sync Project', 'avator-element' );
		}
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'typekit', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'TypeKit Web Fonts by Adobe', 'avator-element' ) . '</h2>';
				esc_html_e( 'TypeKit partners with the world’s leading type foundries to bring thousands of beautiful fonts to designers every day.', 'avator-element' );
			},
			'fields' => [
				self::TYPEKIT_KIT_ID_OPTION_NAME => [
					'label' => __( 'Project ID', 'avator-element' ),
					'field_args' => [
						'type' => 'text',
						'desc' => sprintf( __( 'Enter Your <a href="%s" target="_blank">TypeKit Project ID</a>.', 'avator-element' ), 'https://fonts.adobe.com/typekit' ),
					],
					'setting_args' => [
				...
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-found="%s" data-action="%s" data-nonce="%s" class="button elementor-button-spinner" id="avator_element_typekit_validate_button">%s</button><br><p><span class="avator-element-typekit-data hidden"></span></p>',
	...
	protected function actions() {
		...
		add_action( 'wp_ajax_avator_element_admin_fetch_fonts', [ $this, 'integrations_admin_ajax_handler' ] );
	}
}
