...
class Group_Control_Related extends Group_Control_Query {
	...
	protected function init_fields_by_name( $name ) {
		...
		$fields['post_type']['options']['related'] = __( 'Related', 'avator-element' );
		...
		$related_fallback = [
			...
			'options' => [
				'fallback_none' => __( 'None', 'avator-element' ),
				'fallback_by_id' => __( 'Manual Selection', 'avator-element' ),
				'fallback_recent' => __( 'Recent Posts', 'avator-element' ),
			],
			'default' => 'fallback_none',
			'description' => __( 'Displayed if no relevant results are found. Manual selection display order is random', 'avator-element' ),
			'condition' => [
				'post_type' => 'related',
			],
			'separator' => 'before',
