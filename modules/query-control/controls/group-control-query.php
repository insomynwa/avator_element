<?php
namespace AvatorElement\Modules\QueryControl\Controls;

use Elementor\Controls_Manager;
use Elementor\Group_Control_Base;
use AvatorElement\Classes\Utils;
use AvatorElement\Modules\QueryControl\Module as Query_Module;
...
class Group_Control_Query extends Group_Control_Base {
	...
	protected function init_fields_by_name( $name ) {
		...
		$fields['post_type'] = [
			...
			'options' => [
				'by_id' => __( 'Manual Selection', 'avator-element' ),
				'current_query' => __( 'Current Query', 'avator-element' ),
			],
		];
		...
		$fields['query_include'] = [
			...
			'label' => __( 'Include', 'avator-element' ),
		...
		$fields['include'] = [
			'label' => __( 'Include By', 'avator-element' ),
			'type' => Controls_Manager::SELECT2,
			'multiple' => true,
			'options' => [
				'terms' => __( 'Term', 'avator-element' ),
				'authors' => __( 'Author', 'avator-element' ),
			],
			'condition' => [
				'post_type!' => [
					'by_id',
		...
		$fields['include_term_ids'] = [
			'label' => __( 'Term', 'avator-element' ),
			'description' => __( 'Terms are items in a taxonomy. The available taxonomies are: Categories, Tags, Formats and custom taxonomies.', 'avator-element' ),
			'type' => Query_Module::QUERY_CONTROL_ID,
			'options' => [],
			'label_block' => true,
			'multiple' => true,
		...
		$fields['include_authors'] = [
			'label' => __( 'Author', 'avator-element' ),
			'label_block' => true,
			'type' => Query_Module::QUERY_CONTROL_ID,
			'multiple' => true,
			'default' => [],
		...
		$fields['query_exclude'] = [
			'type' => Controls_Manager::TAB,
			'label' => __( 'Exclude', 'avator-element' ),
			'tabs_wrapper' => $tabs_wrapper,
			'condition' => [
				'post_type!' => [
					'by_id',
		...
		$fields['exclude'] = [
			'label' => __( 'Exclude By', 'avator-element' ),
			'type' => Controls_Manager::SELECT2,
			'multiple' => true,
			'options' => [
				'current_post' => __( 'Current Post', 'avator-element' ),
				'manual_selection' => __( 'Manual Selection', 'avator-element' ),
				'terms' => __( 'Term', 'avator-element' ),
				'authors' => __( 'Author', 'avator-element' ),
			],
			'condition' => [
		...
		$fields['avoid_duplicates'] = [
			'label' => __( 'Avoid Duplicates', 'avator-element' ),
			'type' => Controls_Manager::SWITCHER,
			'default' => '',
			'description' => __( 'Set to Yes to avoid duplicate posts from showing up. This only effects the frontend.', 'avator-element' ),
			'tabs_wrapper' => $tabs_wrapper,
			'inner_tab' => $exclude_wrapper,
			'condition' => [
				'post_type!' => [
		...
		$fields['offset'] = [
			'label' => __( 'Offset', 'avator-element' ),
			...
			'description' => __( 'Use this setting to skip over posts (e.g. \'2\' to skip over 2 posts).', 'avator-element' ),
			'tabs_wrapper' => $tabs_wrapper,
			'inner_tab' => $exclude_wrapper,
		];

		$fields['select_date'] = [
			'label' => __( 'Date', 'avator-element' ),
			'type' => Controls_Manager::SELECT,
			'post_type' => '',
			'options' => [
				'anytime' => __( 'All', 'avator-element' ),
				'today' => __( 'Past Day', 'avator-element' ),
				'week' => __( 'Past Week', 'avator-element' ),
				'month'  => __( 'Past Month', 'avator-element' ),
				'quarter' => __( 'Past Quarter', 'avator-element' ),
				'year' => __( 'Past Year', 'avator-element' ),
				'exact' => __( 'Custom', 'avator-element' ),
			],
			'default' => 'anytime',
			'label_block' => false,
			'multiple' => false,
		...
		$fields['date_before'] = [
			...
			'label_block' => false,
			'multiple' => false,
			'placeholder' => __( 'Choose', 'avator-element' ),
			'condition' => [
				'select_date' => 'exact',
				'post_type!' => [
					'by_id',
			...
			'description' => __( 'Setting a ‘Before’ date will show all the posts published until the chosen date (inclusive).', 'avator-element' ),
		];

		$fields['date_after'] = [
			'label' => __( 'After', 'avator-element' ),
			'type' => Controls_Manager::DATE_TIME,
			'post_type' => '',
			'label_block' => false,
			'multiple' => false,
			...
			'description' => __( 'Setting an ‘After’ date will show all the posts published since the chosen date (inclusive).', 'avator-element' ),
		];
		...
		$fields['orderby'] = [
			...
			'default' => 'post_date',
			'options' => [
				'post_date' => __( 'Date', 'avator-element' ),
				'post_title' => __( 'Title', 'avator-element' ),
				'menu_order' => __( 'Menu Order', 'avator-element' ),
				'rand' => __( 'Random', 'avator-element' ),
			],
			'condition' => [
		...
		$fields['order'] = [
			'label' => __( 'Order', 'avator-element' ),
			'type' => Controls_Manager::SELECT,
			'default' => 'desc',
			'options' => [
				'asc' => __( 'ASC', 'avator-element' ),
				'desc' => __( 'DESC', 'avator-element' ),
			],
			'condition' => [
				'post_type!' => 'current_query',
			],
		...
		$fields['ignore_sticky_posts'] = [
			'label' => __( 'Ignore Sticky Posts', 'avator-element' ),
			'type' => Controls_Manager::SWITCHER,
			'default' => 'yes',
			'condition' => [
				'post_type' => 'post',
			],
			'description' => __( 'Sticky-posts ordering is visible on frontend only', 'avator-element' ),
		];

		$fields['query_id'] = [
			'label' => __( 'Query ID', 'avator-element' ),
			'type' => Controls_Manager::TEXT,
			'default' => '',
			'description' => __( 'Give your Query a custom unique id to allow server side filtering', 'avator-element' ),
			'separator' => 'before',
		];
